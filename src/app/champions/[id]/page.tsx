  ...
  return (
    <div>
      ...
      <div className="mt-6">
        ...
        <li className="text-teal-800">기본 체력: {champion.stats.hp}</li>
        <li className="text-teal-800">기본 마나: {champion.stats.mp}</li>
        <li className="text-teal-800">기본 공격력: {champion.info.attack}</li>
        <li className="text-teal-800">기본 방어력: {champion.info.defense}</li>
        <li className="text-teal-800">기본 주문력: {champion.info.magic}</li>
        <li className="text-teal-800">난이도: {champion.info.difficulty}</li>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 text-center">
        {champion.skins.map((skin: ChampionSkin) => (
          <div key={skin.id}>
            <div className="flex justify-center">
              <Image
                src={`${process.env.NEXT_RIOT_DDRAGON_URL}/cdn/img/champion/splash/${champion.id}_${skin.num}.jpg`}
                alt={skin.name}
                width={200}
                height={200}
              />
            ...
            <p className="text-teal-800 mt-2 text-lg">{skin.name === "default" ? "기본스킨" : skin.name}</p>
          </div>
        ))}
      </div>
    </div>
